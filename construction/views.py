    ...
    if request.method == "POST":
        ...
        if form.is_valid() and form2.is_valid():
            ...
            for f in files:
                ...
                image.save()
            messages.success(
                request, "Your form has been submitted. We'll be in touch soon"
            )
        ...
        if form.is_valid() and form2.is_valid():
            ...
            for i in admin:
                adnin_notif = Notification.objects.create(
                    receiver=i,
                    description=f"Project {project.project} has been updated",
                    url=f"/project/view/{project.id}",
                )
            client_notif = Notification.objects.create(
                receiver=project.client,
                description=f"Project {project.project} has been updated",
                url=f"/myproject/view/{project.id}",
            )
            ...
            if project.pic:
                pic_notif = Notification.objects.create(
                    receiver=project.pic,
                    description=f"Project {project.project} has been updated",
                    url=f"/project/view/{project.id}",
                )
            if project.whm:
                whm_notif = Notification.objects.create(
                    receiver=project.whm,
                    description=f"Project {project.project} has been updated",
                    url=f"/project/view/{project.id}",
                )
    ...
    try:
        ...
        try:
            ...
            context = {
                ...
                "joborder": joborder,
                "ex_inventory": ex_inventory,
            }
            return render(
                request, "backoffice/project_pages/project_detail.html", context
            )
        context = {
            "data": data,
            "data2": data2,
    ...
    try:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                messages.warning(
                    request,
                    "There is an existing Work Progress. Please delete the previous one to create a new Work Progress.",
                )
                ...
                for i in data2:
                    data4 = ProjectProgressDetails.objects.create(
                        projectprogress=data3, scope_of_work=i.scope_of_work
                    )
        ...
        return render(
            request, "backoffice/quotation_pages/quotation_detail.html", context
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
    ...
    def form_valid(self, form):
        ...
        if formset.is_valid():
            ...
            for i in requisitiondetails:
                ...
                data2 = RequisitionDelivery.objects.create(
...
class RequisitionUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        data = super().get_context_data(**kwargs)
        if self.request.POST:
            data["formset"] = RequisitionUpdateFormSet(
    ...
    def form_valid(self, form):
        ...
        if formset.is_valid():
            ...
            for i in requisitiondetails:
                data2 = RequisitionDelivery.objects.create(
                    requisition=i.requisition, articles=i.articles
                )
            formset.save()
            ...
            for i in admin:
                adnin_notif = Notification.objects.create(
                    receiver=i,
                    description=f"{requisition.whm} has updated the requisition on  Project {requisition.project}",
                    url=f"/materials/requisition/{requisition.id}",
                )
            pm_notif = Notification.objects.create(
                receiver=project.pm,
                description=f"{requisition.whm} has updated the requisition on  Project {requisition.project}",
...
def RequisitionListView(request):
    ...
    return render(
        request, "backoffice/requisition_pages/requisition_list.html", context
    )
...
def RequisitionDetailView(request, pk):
    try:
        ...
        data4 = RequisitionDelivery.objects.filter(requisition=data.id).order_by(
            "status", "articles"
        )
        context = {
            "data": data,
        ...
        return render(
            request, "backoffice/requisition_pages/requisition_detail.html", context
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def RequisitionDeleteView(request, pk):
    ...
    return render(
        request, "backoffice/requisition_pages/requisition_delete.html", context
    )
...
def RequisitionActionView(request, pk):
    ...
    if request.method == "POST":
        ...
        if data.status == "Closed" or data.status == "To be Delivered":
            messages.warning(
                request, "Cannot update Requistion because it's already been complied"
            )
            ...
            if formset.is_valid():
                ...
                for i in delivery:
                    if i.status == "To be delivered":
                        ...
                        if item.quantity < i.quantity:
                            messages.error(
                                request,
                                f"Quantity of {i.articles} on Inventory Office are not sufficient to comply requisition. Please update your stocks or canceled the item request.",
                            )
                        ...
                        if item.unit_price is None or item.unit_price == 0:
                            messages.error(
                                request,
                                f'"{i.articles}" has no unit price. Please update its unit price on Material Inventory Office',
                            )
                            return redirect("requisition_action", pk=data.id)
                    elif i.status == "Canceled":
                        i.quantity = 0
                formset.save()
                canceled = RequisitionDelivery.objects.filter(
                    requisition=data.id, status="Canceled"
                )
                # process logic
                for i in data3:
                    if i.status == "To be delivered":
                ...
                whm_notif = Notification.objects.create(
                    receiver=data.whm,
                    description=f"The Admin has complied to your requisition",
                    url=f"/materials/requisition/{data.id}",
                )
    ...
    return render(
        request, "backoffice/requisition_pages/requisition_action.html", context
    )
...
def RequisitionActionView_WHM(request, pk):
    ...
    if request.method == "POST":
        ...
        formset = RequisitionActionFormSet_whm(
            request.POST, instance=data, queryset=queryset
        )
        files = request.FILES.getlist("image")
        if data.status != "Completed":
                ...
                for i in data2:
                    if i.status2 == "Complete":
                        ...
                        if i.quantity < i.quantity2:
                            ...
                            messages.error(
                                request,
                                "Received Quantity cannot be higher than Delivered Quantity ",
                            )
                            ...
                            messages.error(
                                request,
                                'Received Quantity cannot be equal to the Delivered Quantity when selected in action is "Incomplete"',
                            )
                            return redirect("requisition_action_whm", pk=data.id)
                        elif i.quantity2 == 0:
                            for j in data2:
                            ...
                            messages.error(
                                request,
                                'Please check your fields before you submit it"',
                            )
                            return redirect("requisition_action_whm", pk=data.id)
                    else:
                        i.quantity2 = None
                ...
                try:
                    ...
                    for k in data2:
                        if (
                            ...
                            try:
                                data4 = ProjectInventoryDetails.objects.get(
                                    inventory=data3, articles=k.articles, unit=k.unit
                                )
                                data4.quantity += k.quantity2
                                k.save()
                                data4.save()
                            except ObjectDoesNotExist:
                                data4 = ProjectInventoryDetails.objects.create(
                                    inventory=data3,
                                    articles=k.articles,
                                    quantity=k.quantity2,
                                    unit=k.unit,
                                )
                                k.save()
                    ...
                    for f in files:
                        ...
                        image.save()
                    messages.success(
                        request, "Delivered Items has been added to Inventory."
                    )
                    return redirect("requisition_detail", pk=data.id)
                except ObjectDoesNotExist:
                    ...
                    for i in admin:
                        adnin_notif = Notification.objects.create(
                            receiver=i,
                            description=f"{data.whm} has updated the delivery requisition on the project {data.project}",
                            url=f"/materials/requisition/{data.id}",
                        )
                    messages.success(
                        request, "Delivered Items has been added to Inventory."
            ...
            messages.error(
                request, "This Requisition has been already added to inventory"
            )
            return redirect("requisition_detail", pk=data.id)
    else:
    ...
    return render(
        request, "backoffice/requisition_pages/requisition_action_whm.html", context
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for i in admin:
                adnin_notif = Notification.objects.create(
                    receiver=i,
                    description=f"{data.project.pic} has updated the requisition number on the project {data.project}",
                    url=f"/materials/requisition/{data.id}",
                )
...
def ExternalProjectInventoryListView(request):
    ...
    context = {"data2": data2}
    return render(
        request, "backoffice/inventory_pages/externalorder_inventory_list.html", context
    )
...
def ProjectInventoryDetailView(request, pk):
    try:
        ...
        data2 = ProjectInventoryDetails.objects.filter(inventory=data).order_by(
            "articles"
        )
        context = {"data": data, "data2": data2}
        return render(
            request, "backoffice/inventory_pages/inventory_detail.html", context
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def ExternalProjectInventoryDetailView(request, pk):
    try:
        ...
        data2 = ExternalProjectInventoryDetails.objects.filter(inventory=data).order_by(
            "articles"
        )
        context = {"data": data, "data2": data2}
        return render(
            request,
            "backoffice/inventory_pages/externalorder_inventory_detail.html",
            context,
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def ProjectInventoryReport_WHM(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid():
            ...
            for i in formset:
                try:
                    article = ProjectInventoryDetails.objects.get(
                        inventory=data, articles=i.articles
                    )
                except ObjectDoesNotExist:
                    messages.error(
                        request, "Articles on Form does not exist on Inventory"
                    )
                    return redirect("inventory_whm_detail", pk=data.id)
            for i in formset:
                item = ProjectInventoryDetails.objects.get(
                    inventory=data, articles=i.articles
                )
                if item.quantity < i.quantity:
                    messages.error(
                        request,
                        f"Quantity of {i.articles} on Project Site Inventory are not sufficient.",
                    )
                    return redirect("inventory_whm_detail", pk=data.id)
            for i in formset:
                for j in data2:
    ...
    return render(
        request, "backoffice/inventory_pages/inventory_detail_whm.html", context
    )
...
def ExternalProjectInventoryReport_WHM(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and formset.is_valid():
            ...
            for i in formset:
                try:
                    article = ExternalProjectInventoryDetails.objects.get(
                        inventory=data, articles=i.articles
                    )
                except ObjectDoesNotExist:
                    messages.error(
                        request,
                        "Invalid Input. Articles on Form does not exist on External Order Inventory",
                    )
                    return redirect("external_inventory_whm_detail", pk=data.id)
            for i in formset:
                item = ExternalProjectInventoryDetails.objects.get(
                    inventory=data, articles=i.articles
                )
                if item.quantity < i.quantity:
                    messages.error(
                        request,
                        f"Quantity of {i.articles} on Project Site External Order Inventory are not sufficient.",
                    )
                    return redirect("external_inventory_whm_detail", pk=data.id)
            for i in formset:
                for j in data2:
        ...
        for f in formset:
            f.fields[
                "articles"
            ].queryset = ExternalProjectInventoryDetails.objects.filter(inventory=data)
    context = {"data": data, "data2": data2, "form": form, "formset": formset}
    return render(
        request,
        "backoffice/inventory_pages/externalorder_inventory_detail_whm.html",
        context,
    )
...
class ExternalOrderCreateView(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if formset.is_valid():
            ...
            externalorder.save()
            for f in files:
                image = ExternalOrderImage.objects.create(
                    externalorder=externalorder, image=f
                )
            try:
                data2 = ExternalProjectInventory.objects.get(
                    project=externalorder.project
                )
                for i in externalorderdetail:
                    try:
                        data3 = ExternalProjectInventoryDetails.objects.get(
                            articles=i.articles
                        )
                        data3.quantity += i.quantity
                        data3.save()
                    except ObjectDoesNotExist:
                        data3 = ExternalProjectInventoryDetails.objects.create(
                            inventory=data2,
                            articles=i.articles,
                            unit=i.unit,
                            quantity=i.quantity,
                ...
                data2 = ExternalProjectInventory.objects.create(
                    project=externalorder.project
                )
                for i in externalorderdetail:
                    try:
...
def ExternalOrderListView(request):
    ...
    return render(
        request, "backoffice/externalorder_pages/externalorder_list.html", context
    )
...
def ExternalOrderDetailView(request, pk):
    try:
        ...
        return render(
            request, "backoffice/externalorder_pages/externalorder_detail.html", context
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def ExternalOrderDeleteView(request, pk):
    ...
    return render(
        request, "backoffice/externalorder_pages/externalorder_delete.html", context
    )
    ...
    def form_valid(self, form):
        ...
        if formset.is_valid():
            ...
            for i in joborder_task:
                personnel_joborder = PersonnelJobOrder.objects.create(
                    personnel=i.personnel, joborder=i
                )
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            for i in admin:
                adnin_notif = Notification.objects.create(
                    receiver=i,
                    description=f"Job Order status at project {project.project} has been updated",
                    url=f"/task/joborder/{data.id}",
                )
            pm_notif = Notification.objects.create(
                receiver=project.pm,
                description=f"Job Order status at project {project.project} has been updated",
...
def JobOrderDetailView(request, pk):
    try:
        ...
        context = {"data": data, "data2": data2}
        return render(
            request, "backoffice/joborder_pages/joborder_detail.html", context
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def PersonnelCreateView(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "New personnel has been added to the list",
                extra_tags="success",
            )
...
def PersonnelDetailView(request, pk):
    try:
        ...
        context = {"data": data, "data2": data2}
        return render(
            request, "backoffice/personnel_pages/personnel_detail.html", context
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
class ReworkUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        for i in admin:
            adnin_notif = Notification.objects.create(
                receiver=i,
                description=f"Rework at project {rework.project} has been updated",
                url=f"/task/rework/{rework.id}",
            )
        pic_notif = Notification.objects.create(
            receiver=rework.project.pic,
            description=f"Rework at project {rework.project} has been updated",
            url=f"/task/rework/{rework.id}",
        )
        return super().form_valid(form)
...
def ProjectIssuesDeleteView(request, pk):
    ...
    return render(
        request,
        "backoffice/report_pages/project_issue/projectissues_delete.html",
        context,
    )
...
def ProjectIssuesList(request):
    ...
    return render(
        request,
        "backoffice/report_pages/project_issue/projectissues_list.html",
        context,
    )
...
def ProjectIssuesDetailView(request, pk):
    try:
        ...
        return render(
            request,
            "backoffice/report_pages/project_issue/projectissues_detail.html",
            context,
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def dailysitephotosListView(request):
    ...
    context = {"data": data}
    return render(
        request, "backoffice/report_pages/site_photos/sitephotos_list.html", context
    )
...
def dailysitephotosDetailView(request, pk):
    try:
        ...
        return render(
            request,
            "backoffice/report_pages/site_photos/sitephotos_detail.html",
            context,
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            if count != 0:
                client_notif = Notification.objects.create(
                    receiver=project.client,
                    description=f"Site Photos at your project {project.project} has been updated",
                    url=f"/myproject/sitephotos/view/{data.id}",
                )
            return redirect("sitephotos_detail", pk=data.id)
    context = {"formset": formset, "data": data}
    return render(
        request, "backoffice/report_pages/site_photos/sitephotos_update.html", context
    )
...
def dailysitephotosDeleteView(request, pk):
    ...
    context = {"data": data, "data2": data2}
    return render(
        request, "backoffice/report_pages/site_photos/sitephotos_delete.html", context
    )
...
def MaterialReportListView(request):
    ...
    return render(
        request, "backoffice/report_pages/material/materialreport_list.html", context
    )
...
def MaterialReportDetailView(request, pk):
    try:
        ...
        return render(
            request,
            "backoffice/report_pages/material/materialreport_detail.html",
            context,
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def MaterialReportDeleteView(request, pk):
    ...
    context = {"data": data, "data2": data2}
    return render(
        request, "backoffice/report_pages/material/materialreport_delete.html", context
    )
...
def ExternalMaterialReportListView(request):
    ...
    return render(
        request,
        "backoffice/report_pages/external_material/external_report_list.html",
        context,
    )
...
def ExternalMaterialReportDetailView(request, pk):
    try:
        ...
        return render(
            request,
            "backoffice/report_pages/external_material/external_report_detail.html",
            context,
        )
    except ObjectDoesNotExist:
        return render(request, "404.html")
...
def ExternalMaterialReportDeleteView(request, pk):
    ...
    return render(
        request,
        "backoffice/report_pages/external_external_material/external_report_delete.html",
        context,
    )
        ...
        if request.method == "POST":
            ...
            if data.status == "Accepted":
                ...
                for i in admin:
                    adnin_notif = Notification.objects.create(
                        receiver=i,
                        description=f"{project.client} has accepted the quotation on project {project.project}",
                        url=f"/project/quotation/{data.id}",
                    )
                pm_notif = Notification.objects.create(
                    receiver=project.pm,
                    description=f"{project.client} has accepted the quotation on project {project.project}",
                ...
                for i in admin:
                    adnin_notif = Notification.objects.create(
                        receiver=i,
                        description=f"{project.client} has rejected the quotation on project {project.project}",
                        url=f"/project/quotation/{data.id}",
                    )
                pm_notif = Notification.objects.create(
                    receiver=project.pm,
...
def Notification_api(request, pk):
    notification = Notification.objects.filter(receiver_id=pk, is_read=False).order_by(
        "-timestamp"
    )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "requisition" in report:
            try:
                ...
                requisitiondetails = RequisitionDelivery.objects.filter(
        ...
        if "externalorder" in report:
            try:
                externalorder = ExternalOrder.objects.filter(
                    project_id=project.id, date__range=[datefrom, dateto]
                )
                externalorder_details = ExternalOrderDetails.objects.filter(
                    externalorder__in=externalorder
                )
            except ObjectDoesNotExist:
                messages.warning(
                    request, "Input dates does not exist on External Order"
                )
                return redirect("projectreport")
        else:
        ...
        if "sitephotos" in report:
            try:
                sitephotos = SitePhotos.objects.filter(
                    project_id=project.id, date__range=[datefrom, dateto]
                )
                sitephotosdetails = SitePhotosDetails.objects.filter(
        ...
        if "projectissues" in report:
            try:
                projectissues = ProjectIssues.objects.filter(
                    project_id=project.id, date__range=[datefrom, dateto]
                )
            except ObjectDoesNotExist:
                messages.warning(
                    request, "Input dates does not exist on Project Issues"
                )
                return redirect("projectreport")
        else:
        ...
        if "materialreport" in report:
            try:
                materialreport = MaterialReport.objects.filter(
                    project_id=project.id, date__range=[datefrom, dateto]
                )
                materialreportdetails = MaterialReportDetails.objects.filter(
                    report__in=materialreport
                )
            except ObjectDoesNotExist:
                messages.warning(
                    request, "Input dates does not exist on Material Report"
                )
                return redirect("projectreport")
        else:
        ...
        if "externalmaterialreport" in report:
            try:
                externalmaterialreport = ExternalMaterialReport.objects.filter(
                    project_id=project.id, date__range=[datefrom, dateto]
                )
                externalmaterialreportdetails = ExternalMaterialReportDetails.objects.filter(
                    report__in=externalmaterialreport
                )
            except ObjectDoesNotExist:
                messages.warning(
                    request, "Input dates does not exist on External Material Report"
                )
                return redirect("projectreport")
        else:
        ...
        if "projectinventory" in report:
            try:
                ...
                messages.warning(
                    request, "Input dates does not exist on Project Inventory"
                )
                return redirect("projectreport")
        else:
        ...
        if "externalprojectinventory" in report:
            try:
                externalprojectinventory = ExternalProjectInventory.objects.get(
                    project_id=project.id
                )
                externalprojectinventorydetails = ExternalProjectInventoryDetails.objects.filter(
                    inventory_id=externalprojectinventory.id
                )
            except ObjectDoesNotExist:
                messages.warning(
                    request, "Input dates does not exist on External Project Inventory"
                )
                return redirect("projectreport")
        else:
            externalprojectinventory = ExternalProjectInventory.objects.none()
            externalprojectinventorydetails = (
                ExternalProjectInventoryDetails.objects.none()
            )
        ...
        data = {
            ...
            "externalorder_details": externalorder_details,
            "joborder": joborder,
            "jobordertask": jobordertask,
            "rework": rework,
            "projectissues": projectissues,
            "sitephotos": sitephotos,
            "sitephotosdetails": sitephotosdetails,
            "materialreport": materialreport,
            "externalmaterialreport": externalmaterialreport,
            "materialreportdetails": materialreportdetails,
            "externalmaterialreportdetails": externalmaterialreportdetails,
            "projectinventorydetails": projectinventorydetails,
            "externalprojectinventorydetails": externalprojectinventorydetails,
            "projectinventory": projectinventory,
            "externalprojectinventory": externalprojectinventory,
        }
